...
describe('List licenses Test', function() {
 ...
 it('Should return a 200 Ok message. Login ok, admin and list licenses of one organization', function(done) {
  ...
  const user = `email=${email}&password=${password}`;
   chai.request(server)
      ...
      .end((err, res) => {
        chai.request(server)
          .get('/api/organizations/12df8176-0813-49d1-8767-92f4d89f1c15/licenses?page=1')
          .set('Authorization', `bearer ${res.body.token}`)
          .send()
          .end((err, res) => {            
            models.License.findAll({where: {OrganizationId:"12df8176-0813-49d1-8767-92f4d89f1c15"}})
            .then(function(Licenses){
              try{
                res.should.have.status(200);
                res.body.should.have.property('success').eql(true);
                res.body.licenses.length.should.eql(Licenses.length);
                done();
                 } catch(e){
                  done(e);
                }  
